...
RUN GITHUB_REPO="https://github.com/tianon/gosu" \
    && LATEST=$(curl -s https://api.github.com/repos/tianon/gosu/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")') \
...
RUN GITHUB_REPO="https://github.com/mattn/goreman" \
    && LATEST=$(curl -s https://api.github.com/repos/mattn/goreman/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")') \
